...
describe("/posts", () => {
    ...
    test("POST /posts -  Must be able to create a post with an admin user",async () => {
        ...
        const response = await request(app).post('/posts').send(postTest).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
    ...
    test("POST /posts -  should not be able to create a post if not being admin",async () => {
        ...
        const response = await request(app).post('/posts').send(postTest).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
    ...
    test("PATCH /posts/:post_id -  should not be able to update a post if not being admin",async () => {
        ...
        const nonAdminLoginResponse = await request(app).post("/login").send(userNonAdminLogin);
        const response = await request(app).patch(`/posts/${postCreated.id}`).send({content: 'Teste'}).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
        const findPost = await request(app).get(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
    ...
    test("PATCH /posts/:post_id -  Must be able to update a post",async () => {
      ...
      const adminLoginResponse = await request(app).post("/login").send(userAdminLogin);
      const response = await request(app).patch(`/posts/${postCreated.id}`).send({content: 'Testando patch do meu post'}).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
      const findPost = await request(app).get(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
    ...
    test("PATCH - /posts/:post_id  should not be able to update post with invalid id",async () => {
      ...
      const adminLoginResponse = await request(app).post("/login").send(userAdminLogin);
      const response = await request(app).patch(`/posts/1242435`).send({content: 'Nothing'}).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
    ...
    test("GET /posts -  Must be able to list all posts",async () => {
        ...
        const nonAdminLoginResponse = await request(app).post("/login").send(userNonAdminLogin);
        const response = await request(app).get('/posts').set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
    ...
    test("GET /posts/:post_id -  Must be able to get one post",async () => {
        ...
        const nonAdminLoginResponse = await request(app).post("/login").send(userNonAdminLogin);
        const response = await request(app).get(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
    ...
    test("GET /posts/areas/:area_id -  Must be able to list posts by area",async () => {
        ...
        const nonAdminLoginResponse = await request(app).post("/login").send(userNonAdminLogin);
        const response = await request(app).get(`/posts/areas/${marketingAreaCreated.id}`).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
    ...
    test("DELETE /posts/:post_id -  should not be able to delete a post if not being admin",async () => {
        ...
        const nonAdminLoginResponse = await request(app).post("/login").send(userNonAdminLogin);
        const response = await request(app).delete(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
        const findPost = await request(app).get(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${nonAdminLoginResponse.body.token}`)
    ...
    test("DELETE /posts/:post_id -  Must be able to delete a post",async () => {
      ...
      const adminLoginResponse = await request(app).post("/login").send(userAdminLogin);
      const response = await request(app).delete(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
      const findPost = await request(app).get(`/posts/${postCreated.id}`).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
    ...
    test("DELETE - /posts/:post_id  should not be able to delete post with invalid id",async () => {
      ...
      const adminLoginResponse = await request(app).post("/login").send(userAdminLogin);
      const response = await request(app).delete(`/posts/1242435`).send({content: 'Nothing'}).set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
